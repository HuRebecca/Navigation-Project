...
public class Display extends JFrame {
    ...
    public class GraphPanel extends JPanel {
        ...
        public void paintGraph(Graphics g) {
            ...
            Iterator iterator = this.graph.getVertices().iterator();
            while (iterator.hasNext()){
                Vertex currV = (Vertex) iterator.next();
                paintVertex(g, currV);
                for (int i = 0; i < currV.sentEdges.size(); i++){
                    //for each vertex in the graph, use paintVertex(g, v); to print the vertex
                    Edge currE = currV.sentEdges.get(i);
                    if (!edges.contains(currE)){
                        edges.add(currE);
                        // for each edge in the graph,
                        // use paintEdge(g, edge.source, edge.target, edge.distance, Color.LIGHT_GRAY, DEFAULT_THICKNESS, 255);
                        // to print the edge.
                        paintEdge(g, currE.source, currE.target, currE.distance, Color.LIGHT_GRAY, DEFAULT_THICKNESS, 255);
                    }
                }
            }
